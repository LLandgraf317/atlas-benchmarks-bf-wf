...
#ifdef __arm__
  	return syscall(SYS_atlas_submit, tid, 0, static_cast<uint32_t>(id>>32), static_cast<uint32_t>(id&(~0)), exectime, deadline);
#else
...
#ifdef __arm__
  return syscall(SYS_atlas_remove, tid, 0, static_cast<uint32_t>(id>>32), static_cast<uint32_t>(id&(~0)));
#else
...
static inline long atlas_update(pid_t tid, uint64_t id,
                                ...
                                const struct timeval *const deadline) {
#ifdef __arm__
  return syscall(SYS_atlas_update, tid, 0, static_cast<uint32_t>(id>>32), static_cast<uint32_t>(id&(~0)), exectime, deadline);
#else
...
static inline long atlas_tp_destroy(const uint64_t id) {
  ...
  std::cout << "First register: " << static_cast<uint32_t>(id>>32);
  std::cout << ", second register: " << static_cast<uint32_t>(id&(~0)) << std::endl;
#ifdef __arm__
  return syscall(SYS_atlas_tp_destroy, static_cast<uint32_t>(id>>32), static_cast<uint32_t>(id&(~0)));
#else
...
#ifdef __arm__
  return syscall(SYS_atlas_tp_join, 0, static_cast<uint32_t>(id>>32), static_cast<uint32_t>(id&(~0)));
#else
...
static inline long atlas_tp_submit(const uint64_t tpid, const uint64_t id,
                                   ...
                                   const struct timeval *const deadline) {
#ifdef __arm__
  return syscall(SYS_atlas_tp_submit, tpid, 0, static_cast<uint32_t>(id>>32), static_cast<uint32_t>(id&(~0)), exectime, deadline);
#else
...
static inline decltype(auto) destroy(const uint64_t id) {
  std::cout << "Destroyed threadpool!" << std::endl;
#ifdef __arm__
  return syscall(SYS_atlas_tp_destroy, static_cast<uint32_t>(id&(~0)), static_cast<uint32_t>(id>>32));
#else
  return syscall(SYS_atlas_tp_destroy, id);
#endif
}
...
#ifdef __arm__
  return syscall(SYS_atlas_tp_join, static_cast<uint32_t>(id&(~0)), static_cast<uint32_t>(id>>32));
#else
  return syscall(SYS_atlas_tp_join, id);
#endif
}
...
static inline decltype(auto) submit(const uint64_t tpid, const uint64_t id,
                                    const struct timeval *const exectime,
                                    const struct timeval *const deadline) {
  return syscall(SYS_atlas_tp_submit, 0, 
	tpid, 
	id, exectime, deadline);
}
